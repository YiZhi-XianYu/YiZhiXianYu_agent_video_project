...
本地 Compose 通过 `workers` profile 提供 `tool-worker-media`、`tool-worker-model` 和 `tool-worker-render`。多个 Worker 连接同一队列即可分摊任务；`prefetch` 控制每个 Worker 预取数量，避免重任务被某个实例长期占住。
...
Tool 完成后优先回调 Control Plane；如果回调丢失，Control Plane Poller 会查询 Tool Service 状态并补写结果。两条路径都经过幂等处理。
