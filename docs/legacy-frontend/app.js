const state = { projectId: null, assetId: null, assetIds: [], assets: [], workflowRunId: null, timer: null,
    ...
    renderedAssetHash: "", renderedTaskHash: "",
    renderedShotKey: "", renderedDecisionHash: "", taskProgressCache: new Map(),
    renderedMetadataDone: false, renderedProxyIds: new Set(), proxyDone: false };
...
el("load-workflow").addEventListener("click", async () => {
    ...
    try {
        ...
        state.renderedProxyIds = new Set();
        state.proxyDone = false;
...
async function refreshRun() {
    ...
    try {
        ...
        // Proxy: complete divorce from task polling — extract only until all rendered
        if (state.renderedProxyIds.size === 0 || !state.proxyDone) {
            const px = (run.tasks || []).flatMap(t =>
                (t.artifacts || []).filter(a => a.type === "VIDEO_PROXY").map(a => ({...a, taskAssetId: t.assetId}))
            );
            if (px.length) appendProxies(px);
            if (px.length > 0 && px.every(p => state.renderedProxyIds.has(p.id))) {
                state.proxyDone = true;
            }
        }
        renderRun(run);
        if (["SUCCEEDED", "FAILED"].includes(run.status)) {
...
function renderRun(run) {
    if (!state.lastRunStatus || state.lastRunStatus !== run.status) {
        state.lastRunStatus = run.status;
        setStatus(el("run-status"), run.status);
        el("run-status").textContent = run.status;
    }
    // renderAssets: guard by asset id hash (assets are immutable after upload)
    const ah = (run.assets || state.assets).map(a => a.id).sort().join("|");
    if (ah !== state.renderedAssetHash) { state.renderedAssetHash = ah; renderAssets(run.assets || state.assets); }

    // renderTasks: guard by task id:status hash (progress handled separately)
    const th = (run.tasks || []).map(t => t.id + ":" + t.status).sort().join("|");
    if (th !== state.renderedTaskHash) { state.renderedTaskHash = th; renderTasks(run.tasks || []); }
    updateTaskProgress(run.tasks || []);
    ...
    // renderShots: guard by shot count + artifact type presence
    const sc = (run.tasks || []).reduce((sum, task) => {
        const sl = (task.artifacts || []).find(a => a.type === "SHOT_LIST");
        if (!sl) return sum;
        try { return sum + (JSON.parse(sl.metadataJson)?.shots?.length || 0); } catch { return sum; }
    }, 0);
    const atypes = ["SHOT_QUALITY", "SHOT_RANKING", "SCENE_TAGS", "OBJECT_TAGS", "PERSON_TAGS", "HIGHLIGHT_SET"];
    const aexist = atypes.map(t => (run.tasks || []).some(task => (task.artifacts || []).some(a => a.type === t)) ? 1 : 0).join("");
    const sk = sc + "|" + aexist;
    if (sk !== state.renderedShotKey) { state.renderedShotKey = sk; renderShots(run.tasks || []); }

    // renderDecisions: guard by decision artifact id:type:metadataLength hash
    const dtypes = ["SHOT_RANKING", "STORY_PLAN", "HIGHLIGHT_SET", "TIMELINE"];
    const dh = (run.tasks || []).flatMap(t => t.artifacts || [])
        .filter(a => dtypes.includes(a.type))
        .map(a => a.id + ":" + a.type + ":" + (a.metadataJson || "").length)
        .sort().join("|");
    if (dh !== state.renderedDecisionHash) { state.renderedDecisionHash = dh; renderDecisions(run.tasks || []); }
...
function renderTasks(tasks) {
    ...
    // Map existing groups by heading text
    const existingGroups = new Map();
    grid.querySelectorAll(".task-group").forEach(g => {
        const h = g.querySelector(".task-group-title");
        if (h) existingGroups.set(h.textContent, g);
    });
    ...
    grouped.forEach((assetTasks, assetId) => {
        const asset = state.assets.find(item => item.id === assetId);
        const groupName = asset?.fileName || "Workflow";
        seenGroupNames.add(groupName);

        let group = existingGroups.get(groupName);
        if (!group) {
        ...
        // Map existing cards in this group
        const existingCards = new Map();
        group.querySelectorAll(".task-card[data-task-id]").forEach(c => {
            existingCards.set(c.dataset.taskId, c);
        });
        const seenCardIds = new Set();

        assetTasks.forEach(task => {
            seenCardIds.add(task.id);
            let card = existingCards.get(task.id);
            if (card) {
                // Update existing card in-place
                const st = card.querySelector(".status");
                if (st) { setStatus(st, task.status); st.textContent = task.status; }
                const bar = card.querySelector(".progress span");
                if (bar) bar.style.width = task.progress + "%";
                const metas = card.querySelectorAll(".node-meta span");
                if (metas.length >= 2) {
                    metas[0].textContent = task.progress + "%";
                    const rl = task.retryCount ? ` · 重试 ${task.retryCount}` : "";
                    metas[1].textContent = `尝试 ${task.attempt}${rl}`;
                }
            } else {
                // Create new card — same structure as original
                ...
                const rl = task.retryCount ? ` · 重试 ${task.retryCount}` : "";
                card.innerHTML = `<div class="node-topline"><span class="node-icon">${task.nodeKey.slice(0, 2).toUpperCase()}</span></div><h3>${task.nodeKey}</h3><p>${asset?.fileName || "Workflow"}</p><p><code>${task.toolName}@${task.toolVersion}</code></p><div class="progress"><span style="width:${task.progress}%"></span></div><div class="node-meta"><span>${task.progress}%</span><span>尝试 ${task.attempt}${rl}</span></div>`;
                card.querySelector(".node-topline").append(status);
            }
            group.appendChild(card);
        });

        // Remove stale cards from this group
        existingCards.forEach((card, id) => {
            if (!seenCardIds.has(id)) card.remove();
        });
    });

    // Remove stale groups
    existingGroups.forEach((group, name) => {
        if (!seenGroupNames.has(name)) group.remove();
    });
}

function updateTaskProgress(tasks) {
    tasks.forEach(task => {
        const prev = state.taskProgressCache.get(task.id);
        if (prev && Number(prev.progress) === Number(task.progress) && Number(prev.attempt) === Number(task.attempt)) return;
        state.taskProgressCache.set(task.id, { progress: task.progress, attempt: task.attempt });
...
function renderMetadata(artifact) {
    if (state.renderedMetadataDone) return;
    state.renderedMetadataDone = true;
...
function appendProxies(proxies) {
    ...
    const fresh = proxies.filter(p => !state.renderedProxyIds.has(p.id));
    if (!fresh.length) return;
    el("proxy-artifact-state").textContent = "已生成";
    fresh.forEach(proxy => {
        state.renderedProxyIds.add(proxy.id);
