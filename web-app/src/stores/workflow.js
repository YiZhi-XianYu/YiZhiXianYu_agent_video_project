...
export const useWorkflowStore = defineStore('workflow', () => {
    ...
    const currentGate = computed(() => {
        if (!run.value?.currentGateKey || !run.value.gates)
            return null;
    ...
    /** 已完成 Task 数量 */
    const completedTaskCount = computed(() => tasks.value.filter((t) => t.status === 'SUCCEEDED').length);
    /** Task 总数 */
    ...
    // ============================================================
    /** 刷新 Workflow 运行详情 */
    async function fetchRun(workflowRunId) {
        error.value = null;
        try {
            run.value = await api.get(`/api/v1/workflow-runs/${workflowRunId}`);
        }
        catch (e) {
    ...
    async function continueWorkflow(workflowRunId) {
        error.value = null;
        try {
